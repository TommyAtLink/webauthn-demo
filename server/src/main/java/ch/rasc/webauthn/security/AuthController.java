...
public class AuthController {
    ...
    public RegistrationStartResponse registrationStart(
            ...
            @RequestParam(name = "registrationAddToken", required = false) String registrationAddToken,
        ...
        if (username != null && !username.isEmpty()) {
            ...
            int count = this.dsl.selectCount().from(APP_USER).where(APP_USER.USERNAME.equalIgnoreCase(username)).fetchOne(0, int.class);
            if (count > 0) {
                return new RegistrationStartResponse(RegistrationStartResponse.Status.USERNAME_TAKEN);
            }
